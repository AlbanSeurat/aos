...
impl TableDescriptor {
    pub fn new(next_lvl_table_addr: usize) -> Result<TableDescriptor, &'static str> {
        ...
        let shift = next_lvl_table_addr >> FOUR_KIB_SHIFT;
        Ok(TableDescriptor(
            STAGE1_DESCRIPTOR::VALID::True
                + STAGE1_DESCRIPTOR::TYPE::Table
                + STAGE1_DESCRIPTOR::NEXT_LVL_TABLE_ADDR_4KiB.val(shift as u64),
        ))
    }
...
impl Lvl2BlockDescriptor {
    pub fn new(
        ...
        let shift = output_addr >> TWO_MIB_SHIFT;
        Ok(Lvl2BlockDescriptor(
            STAGE1_DESCRIPTOR::VALID::True
                + STAGE1_DESCRIPTOR::AF::True
                + into_mmu_attributes(attribute_fields)
                + STAGE1_DESCRIPTOR::TYPE::Block
                + STAGE1_DESCRIPTOR::LVL2_OUTPUT_ADDR_4KiB.val(shift as u64),
        ))
    }
...
impl PageDescriptor {
    pub fn new(
        ...
        let shift = output_addr >> FOUR_KIB_SHIFT;
        Ok(PageDescriptor(
            STAGE1_DESCRIPTOR::VALID::True
                + STAGE1_DESCRIPTOR::AF::True
                + into_mmu_attributes(attribute_fields)
...
fn into_mmu_attributes(
    ...
    use crate::memory::mapping::{AccessPermissions, MemAttributes};
